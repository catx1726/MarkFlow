...
| 2026-08-21 12:00:00 | [Superseded by 12:30 entry] Two-level sticky fixes: header top-4 + page-section sticky | src/sidepanel/** | Header sticks at initial position (no 16px drift); page headers pin under folder row via measured --folder-row-h var; z-index stack 40/30/20 | HEAD | git revert HEAD |
| 2026-08-21 12:30:00 | Sticky round-2: no bleed-through, no jump, layer tint | src/sidepanel/** | top padding moved into header (bg covers to viewport top, top-0, no +16 hack); removed -mx width compensation causing stick-jump; page header bg-gray-50 for layer separation | HEAD | git revert HEAD |
| 2026-08-21 13:00:00 | Round-14 CR: ResizeObserver for folder row height | src/sidepanel/components/{TagFolder,PageSection}.vue | B1: row height now continuously measured (font-load/resize safe); B2: var-source comments, provide/inject declined as overkill; changelogs annotated superseded | HEAD | git revert HEAD |
...
| 2026-08-21 13:30:00 | Chapter-level (group header) sticky | src/sidepanel/components/PageSection.vue | 4-level sticky stack complete: header z-40 > folder z-30 > page z-20 > group z-10; --page-header-h measured at runtime | HEAD | git revert HEAD |
| 2026-09-07 22:15:00 | Disable vue/custom-event-name-casing lint rule | eslint.config.mjs | kebab-case custom events are this repo's convention; rule misfires on Vue template listeners (PR #72 review verdict); unblocks lint-staged pre-commit | 2038436 | git revert 2038436 |
| 2026-09-07 22:10:00 | Align owned-page texture with landing page | src/{popup,sidepanel,options}/** + src/styles/main.css + unocss.config.ts | Bresson-style alignment: gray-→neutral- (145 sites), removed all 12 shadow-*, rounded-lg→md (11), hairline borders for Options cards; dropped dead unocss color tokens; injected Tooltip/Modal keep elevation per Driver decision | 71101e0 | git revert 71101e0 |
| 2026-09-07 22:20:00 | Sidepanel title interaction & page-title fix | src/logic/tagTree.ts + src/contentScripts/restorer.ts + src/sidepanel/components/{TagFolder,PageSection,MarkItem}.vue | select-none on four clickable row levels (no more accidental text selection); pageTitle picks first mark with title; legacy marks self-heal via document.title backfill on revisit (idempotent, once per URL per session); +4 TDD tests | 3acf6e3 | git revert 3acf6e3 |
