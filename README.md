...
- **🏷️ 标签化知识管理**: 为标记添加标签/课题维度，跨网页构建个人知识图谱。三级树状侧边栏（**标签 → 网页 → 标记分组**）让碎片信息有机归集，告别“滚动地狱”。
- **📍 书签式精准跳转**: 每一处高亮都是一个“锚点”。通过侧边栏点击笔记，瞬间回跳原文位置。
- **📂 自动大纲构建**: 智能识别网页 H1-H6 标题，将你的**标记**按章节自动归类。
- **📊 智能排序**: 标记按文档物理位置自动排序（即使先标后段也能正序显示），网页按最近活跃度置顶，回顾路径与阅读路径完全一致。
...
**Q: 标签系统怎么用？我可以给一个标记打多个标签吗？** A: 高亮后，在浮动工具栏中点击“添加标签”即可为当前标记关联标签。一个标记可以属于多个标签，它会在每个对应的标签文件夹下出现。未打标签的标记默认存放在“收集箱”中。

**Q: 导出的 Markdown 可以直接粘贴到 Obsidian/Notion 吗？** A: 可以。导出内容使用粗体和引用块代替深层标题，零层级入侵，可无缝嵌入到任意已有文档的任何位置。
...
| **Level 3**   | **共识重构** | Dual-Anchor Consensus + 上下文校验      | 唯一项且共识得分 >= 85 且上下文相似度 >= 75% | 内容增删场景（双向边界投票 + 块级边界约束） |
| **Level 4**   | **歧义消除** | Disambiguation UI                       | 用户手动交互                            | 最终兜底，由用户进行物理校准                |

> **注意**：当共识得分 >= 90 时，自动更新数据库中的序列化路径，防止临时文档漂移导致永久错位。
