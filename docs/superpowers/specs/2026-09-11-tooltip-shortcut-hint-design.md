...
- **Options「重新显示」**（2026-09-11 验收修订）：重置**两个**一次性标志（`coachTipDone` + `tooltipShortcutHintDone`）；状态行与 disabled 判定统一为「**都从未显示**才为 未显示/禁用」（判定函数 `isReshowDisabled`——首版曾把方向写反为「都已显示→禁用」，已修复并加回归测试）
