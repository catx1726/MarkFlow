...
**目标**
- 建立 i18n 基础设施：TS 字典 + `t()` 函数 + 自动语言检测
- 扩展全部用户可见文案英文化（zh 保持现状为源语言，en 为翻译目标）
- manifest name/description 双语（`_locales`）
- 宣传页英文版（`docs/en/index.html` 或 query 切换，落地方式在实施时定）

**非目标（YAGNI）**
- ❌ 不做语言手动切换开关——跟随浏览器 UI 语言（`browser.i18n.getUILanguage()`），zh* → 中文，其余 → 英文
...
**PR-2：宣传页英文版**
- `docs/en/index.html`（英文翻译版），`docs/index.html` 顶部加语言切换链接；og:locale 等 meta 适配
