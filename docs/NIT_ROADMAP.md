...
> **2026-09-10 追加**：AI 录制宣传片实操产品全链路 + 代码级功能盘点后，识别出一批"外壳层"短板（onboarding / 撤销 / 备份 / 失败可见性等），作为"产品实测分析"批次汇入第 3 节与第 6 节。其中恢复歧义相关条目**明确遵守 SPEC-2026-06-26-001**（Level 3/4 有意跳过），只做现行"侧边栏保留上下文"方案的可见性增强。
...
| **页面内首次引导浮层（Coach Tip）** | 产品实测分析 2026-09-10 | 中 | 极高 | [已完成] | `Alt`+划词是核心手势但零发现机制，新用户安装后不知其存在。方案：首次**未按 Alt** 划词时，选区旁出现一次性轻提示"按住 Alt 划词即可标记"；设置项记录已读、可跳过。Chrome 上架前转化率最高的准备。2026-09-11 已实现：CoachTip.vue 一次性提示（显示即置位/Alt 成功标记即置位）+ Options 重显入口，Spec: docs/superpowers/specs/2026-09-11-coach-tip-design.md |
...
| **本地 JSON 备份/导入** | 产品实测分析 2026-09-10 | 低 | 高 | ⭐⭐⭐⭐⭐ | "本地优先"缺"本地可备份"这条腿：不使用 GitHub 的用户卸载即丢数据。方案：Options 增加全量导出（marks+tags+settings 带版本号）与导入（格式校验 + 复用 `sync.ts` 的 merge 合并策略）。 |
| **恢复失败标记的可见性与再标记流** | 产品实测分析 2026-09-10 | 低-中 | 高 | ⭐⭐⭐⭐ | **遵守 SPEC-2026-06-26-001，不重启 Level 3/4 歧义 UI**。强化现行"侧边栏保留上下文"方案：①失败标记侧边栏加琥珀描边/图标区分 ②"复制原文"按钮便于用户 Ctrl+F 手动定位 ③Popup 显示失败计数引导查看。把"静默失败"变为"可操作的失败"。 |
| **快捷键体系修复（Alt+S 冲突 + manifest commands）** | 产品实测分析 2026-09-10 | 低 | 中 | ⭐⭐⭐⭐ | `config.ts:35` openSidePanel 的 `Alt+S` 与默认保存快捷键重复定义且未实际接线（`index.ts:105-110` 仅 preventDefault）。方案：manifest 增加 `commands` 字段注册浏览器级快捷键，Options 快捷键自定义区联动提示；消除重复定义。 |
| **设置即改即存** | 产品实测分析 2026-09-10 | 低-中 | 中 | ⭐⭐⭐ | Options 显式保存模型与主流扩展"即改即存"习惯不符，忘点保存丢改动。方案：watch + 防抖自动保存，保留"已保存"反馈；Gist Token 等敏感项仍需显式确认。 |
| **同步状态可见性** | 产品实测分析 2026-09-10 | 低 | 中 | ⭐⭐⭐ | 同步状态仅在 Options 深处显示。侧边栏 Header / Popup 增加状态点（绿/灰/红），失败时点击跳转 Options 同步分区；速率限制退避对用户可见。 |
...
| **Chromium 商店上架** | 分析 | 待办 | - |
| **页面内首次引导浮层（Coach Tip）** | 产品实测分析 2026-09-10 | 已完成 | 外壳 Sprint 起点，方案详见第 3 节 |
...
| 项目 | 渠道 | 成本 | 收益 | 优先级 | 要点 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **商店 listing 标题关键词卡位** | AMO / 未来 CWS | 低 | 极高 | P0（文案已备好，待 Driver 粘贴） | 商店搜索是扩展最大自然流量入口，标题权重最高。⚠️ 2026-09-13 修正：本行旧提议「MarkFlow: 网页高亮 + 精准回跳」含降调红线词（写于红线确立前），**作废**；新文案见 `docs/store-listing.md`（zh「MarkFlow: 网页高亮与笔记，回跳原文」/ en「MarkFlow: Web Highlighter & Notes」），AMO 现行描述中「精准跳转」「瞬间回跳」红线词一并清理；CWS 上架取同构文案 |
