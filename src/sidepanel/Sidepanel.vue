...
<template>
  ...
  >
    ...
    <!-- 标签创建 UI -->
    <div class="px-2 mb-6">
      <div class="flex gap-2 bg-white dark:bg-gray-800 p-2 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700">
        <input 
          v-model="newTagName" 
          placeholder="新建课题标签..." 
      ...
      <div v-else>
        ...
        <div v-for="[tagId, folder] in Object.entries(structuredMarks)" :key="tagId" class="mb-6">
          ...
          >
            ...
            <span class="text-xs text-gray-400">({{ Object.keys(folder.pages).length }})</span>
          </div>

          <div v-if="!isFolderCollapsed(tagId)" class="space-y-4 p-2 border-x border-b border-gray-200 dark:border-gray-700 rounded-b-lg bg-gray-50/50 dark:bg-gray-800/30">
            <section
                          ...
                          >
                            <div class="py-1">
                              <div class="px-4 py-2 border-b border-gray-100 dark:border-gray-600">
                                <p class="text-[10px] text-gray-400 uppercase font-bold mb-1">标签关联</p>
                                <div class="flex flex-wrap gap-1 max-h-24 overflow-y-auto">
                                  <button
                                    v-for="tag in Object.values(tagsMetadata)"
                                    :key="tag.id"
                                    class="text-[10px] px-2 py-0.5 rounded-full border transition-colors"
                                    :class="mark.tags?.includes(tag.id) ? 'bg-blue-100 border-blue-300 text-blue-800' : 'bg-gray-50 border-gray-200 text-gray-600'"
                                    @click="toggleTagForMark(mark, tag.id)"
                                  >
                                    {{ tag.name }}
                                  </button>
                                  <p v-if="Object.keys(tagsMetadata).length === 0" class="text-[10px] text-gray-400 italic">请先创建标签</p>
                                </div>
                              </div>
                              <button
                              ...
                              <button
                                class="w-full text-left px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/50"
