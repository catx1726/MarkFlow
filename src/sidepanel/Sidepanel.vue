...
import { tagsMetadata, tagsReady } from '~/logic/storage'
...
// State for collapsing folders, URLs and groups
const collapsedFolders = ref<Record<string, boolean>>({}),
  ...
  structuredMarks = computed(() => {
    // 最终结构: Record<TagID, { tagName: string, pages: Record<Url, { pageTitle: string, groups: MarkGroup[] }> }>
    const tree: Record<string, { tagName: string; pages: Record<string, { pageTitle: string; groups: MarkGroup[]; totalMarks: number }> }> = {
    ...
    for (const { url, marks } of sortedUrls) {
      ...
      for (const mark of marks) {
        ...
        for (const tagId of targetTags) {
          // 如果标签元数据中没有此 ID (可能是旧数据或已删除标签)，归类到 inbox
...
<template>
  ...
  >
    ...
    <div class="space-y-6">
      ...
      <div v-else>
        <!-- 顶级文件夹层 (Tags / Inbox) -->
        <div v-for="[tagId, folder] in Object.entries(structuredMarks)" :key="tagId" class="mb-4">
          <div 
            v-if="Object.keys(folder.pages).length > 0"
            class="flex items-center gap-2 p-2 bg-gray-200 dark:bg-gray-700 rounded-t-lg cursor-pointer hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"
          ...
          >
            ...
            <span class="font-bold text-gray-700 dark:text-gray-200">{{ folder.tagName }}</span>
            <span class="text-xs text-gray-400">({{ Object.keys(folder.pages).length }} 页面)</span>
          </div>

          <div v-if="!isFolderCollapsed(tagId)" class="space-y-4 p-2 border-x border-b border-gray-200 dark:border-gray-700 rounded-b-lg">
            <section
