...
async function createTag() {
  ...
  const tag = { id, name: newTagName.value.trim(), color: '#3B82F6', isAutoGenerated: false, createdAt: Date.now() }
  tagsMetadata.value = { ...tagsMetadata.value, [id]: tag }
  newTagName.value = ''
}
...
function confirmRename() {
  if (editingTagId.value && editingTagName.value.trim()) {
    const tag = tagsMetadata.value[editingTagId.value]
    if (tag) {
      tagsMetadata.value = {
        ...tagsMetadata.value,
        [editingTagId.value]: { ...tag, name: editingTagName.value.trim() }
      }
    }
  }
