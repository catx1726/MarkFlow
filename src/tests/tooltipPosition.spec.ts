...
describe('computeTooltipPosition', () => {
  ...
  it('鼠标感知：正向划选（指针在选区下半）放下方且水平跟随鼠标', () => {
    ...
    const pos = computeTooltipPosition(anchor, TOOLTIP, VIEWPORT, 8, 8, { x: 460, y: 118 })
  ...
  it('鼠标感知：反向划选（指针在选区上半）放上方', () => {
    ...
    const pos = computeTooltipPosition(anchor, TOOLTIP, VIEWPORT, 8, 8, { x: 300, y: 505 })
  ...
  it('鼠标感知：指针贴近右缘时水平钳制不溢出', () => {
    const anchor = { top: 100, left: 200, width: 300, height: 24 }
    const pos = computeTooltipPosition(anchor, TOOLTIP, VIEWPORT, 8, 8, { x: 1270, y: 118 })
  ...
  it('鼠标感知：首选侧空间不足时自动翻转', () => {
    ...
    const pos = computeTooltipPosition(anchor, TOOLTIP, VIEWPORT, 8, 8, { x: 300, y: 720 })
