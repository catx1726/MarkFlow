...
onMessage('get-all-tags', async () => {
  // eslint-disable-next-line no-console
  console.log('[DIAG background get-all-tags] received')
  await ensureReady()
  // eslint-disable-next-line no-console
  console.log('[DIAG background get-all-tags] ready, returning', Object.keys(tagsMetadata.value || {}).length, 'tags')
...
onMessage<{ name: string, color?: string }>('create-tag', async ({ data }) => {
  // eslint-disable-next-line no-console
  console.log('[DIAG background create-tag] received, name=', data?.name)
  await ensureReady()
  try {
    ...
    // eslint-disable-next-line no-console
    console.log('[DIAG background create-tag] enqueueWrite start')
    await enqueueWrite(async () => {
      tagsMetadata.value = { ...tagsMetadata.value, [id]: newTag }
    })
    // eslint-disable-next-line no-console
    console.log('[DIAG background create-tag] enqueueWrite done, returning')
    return newTag
  }
  catch (error) {
